...
describe UsersController do
  ...
  describe 'mark completed' do
    params = { participant_id: 5,  group: 3, available_payments: 5, job: "a" }
    u = User.new(participant_id: params[:participant_id], group: params[:group], available_payments: params[:available_payments], job: params[:job])
  ...
  describe 'POST create' do
    describe 'with valid params' do
      it 'assigns a newly created user as @user' do
        allow(User).to receive(:new).with(participant_id: nil, group: nil, available_payments: nil, job: nil) { mock_user(save: true) }
        post :create, user: { participant_id: nil, group: nil, available_payments: nil, job: nil }
        expect(assigns(:user)).to be(mock_user)
      end
    ...
    describe 'with invalid params' do
      it 'assigns a newly created but unsaved user as @user' do
        allow(User).to receive(:new).with(participant_id: nil, group: nil, available_payments: nil, job: nil) { mock_user(save: false) }
        post :create, user: { participant_id: nil, group: nil, available_payments: nil, job: nil }
        expect(assigns(:user)).to be(mock_user)
      end
