...
describe Response do
  ...
  describe "update actions and set total_time" do
    ...
    it "first create" do
      expect(@response_1.actions).to eq [{last_response: nil, time_since_last_action: truncate_to(@time_now - @round.created_at, 6), correct?: true, time_of_action:  @time_now}.to_json]
      expect(@response_1.total_time_to_edit).to eq truncate_to(@time_now - @round.created_at, 6)
    end

    it "second create" do
      expect(@response_2.actions).to eq [{last_response: @response_1.id, time_since_last_action: truncate_to(@time_now - @response_1.created_at, 6), correct?: false, time_of_action:  @time_now}.to_json]
    end

    it "on update" do
      @response_2.update(correct_answer: "no", corrected: "no")
      expect(@response_2.actions).to eq [{last_response: @response_1.id, time_since_last_action: truncate_to(@time_now - @response_1.created_at, 6), correct?: false, time_of_action:  @time_now}.to_json, {last_response: @response_2.id, time_since_last_action: truncate_to(@time_now - @response_2.created_at, 6), correct?: true, time_of_action:  @time_now}.to_json]
