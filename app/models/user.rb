    ...
    CSV.generate do |csv|
      ...
      users.each do |user|
        csv << [
          ...
          user.rounds.where(round_number: 1).pluck(:time_to_complete_in_seconds),
          user.rounds.where(round_number: 2).pluck(:time_to_complete_in_seconds),
          user.rounds.where(round_number: 3).pluck(:time_to_complete_in_seconds),
          user.rounds.where(round_number: 4).pluck(:time_to_complete_in_seconds),
